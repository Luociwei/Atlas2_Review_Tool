...
(function ($) {
	$.fn.fancyTable = function (options) {
		...
		this.tableUpdate = function (elm) {
			...
			$(elm).find('.search_result').html(`${elm.fancyTable.matches} results`);
		...
		this.each(function () {
			...
			if (settings.searchable) {
				...
				if (settings.globalSearch) {
					var selecrField = $('<select title="PerPage" class="custom-select custom-select-sm perpage"></select>')
					for (var i=10; i<101; i+=10) {
						if (i==settings.perPage)
							selecrField.append(`<option value=${i} selected>${i}</option>`)
						else
							selecrField.append(`<option value=${i}>${i}</option>`)
					}
					...
					var search_result = $("<p class='search_result'>0 results</p>")
					var th = $("<th>", { style: "padding:2px;position:relative;border-top:0" }).attr("colspan", elm.fancyTable.nColumns);
					$(th).append('<label>PerPage:</label>');
					$(selecrField).appendTo($(th));
					$(searchField).appendTo($(th));
					$(search_result).appendTo($(th));
