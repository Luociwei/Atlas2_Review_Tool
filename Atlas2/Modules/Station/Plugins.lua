...
function clearAndBackupFixtureLog( )
    ...
    -- Log.LogInfo("$$$$ debug_test--clearAndBackupFixtureLog----")
    local fixtureBuilder = Atlas.loadPlugin("FixturePlugin")
    local fixturePlugin = fixtureBuilder.createFixtureBuilder(Group.index-1)

    local verdor_id = fixturePlugin.getVendor()
    -- Log.LogInfo("$$$$ debug_test--fixturePlugin.getVendor:"..verdor_id..'----')

    if string.lower(verdor_id) == 'suncode' then 
        executeShellCommand("cp -r /vault/Atlas/FixtureLog/SunCode /vault/Atlas/FixtureLog_BK")
        executeShellCommand("rm -r /vault/Atlas/FixtureLog/SunCode/*")

        -- Log.LogInfo("$$$$ debug_test--clear suncode fixture log..-----")
    else
        ...
        -- Log.LogInfo("$$$$ debug_test--clear Microtest fixture log..-----")
...
function Plugins.getSlots(groupPlugins)
    ...
    local slots = Plugins.getSlotsByInteractiveView(groupPlugins, viewConfigInput)
