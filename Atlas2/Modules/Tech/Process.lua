...
function Process.checkExpect( paraTab )
    ...
    local raiseErrorWhenNotFound = paraTab.AdditionalParameters.raiseErrorWhenNotFound
    local result = "TRUE"
    if string.find(ret, expect) == nil then result = "FALSE" end
    if raiseErrorWhenNotFound and raiseErrorWhenNotFound == "YES" and result == "FALSE" then
        ...
        error(expect .. " not find in response")
    elseif failWhenNotFound and failWhenNotFound == "YES" and result == "FALSE" then
        Record.createBinaryRecord(false, paraTab.Technology, subtestname, paraTab.AdditionalParameters.subsubtestname, expect .. " not find in response")
    else
