...
-- DUTInfo.writeAndCompareSN( paraTab )
...
function DUTInfo.writeAndCompareSN( paraTab )
    ...
    if paraTab.AdditionalParameters.attribute then
        DataReporting.submit( DataReporting.createAttribute( paraTab.AdditionalParameters.attribute, sn ) )
    end
...
-- DUTInfo.writeAndCompareCFG( paraTab )
...
function DUTInfo.writeAndCompareCFG( paraTab )
    ...
    if cfg ~= nil and cfg ~= "" then
        if paraTab.AdditionalParameters.attribute then
            DataReporting.submit( DataReporting.createAttribute( paraTab.AdditionalParameters.attribute, cfg ) )
        end
...
-- DUTInfo.setTypeCondition( paraTab )
-- Function to Check MLB Type with the input string or restore device log
-- Created By : Jayson Ye
-- Initial Creation Date : 15/06/2021
-- Modified By : N/A
-- Modification Date : N/A
...
-- Primary Usage : DFU
-- Input Arguments : param table
-- Output Arguments : string("MLB_A"/"MLB_B")
-----------------------------------------------------------------------------------]]
...
-- DUTInfo.getStorageSize( paraTab )
...
function DUTInfo.getStorageSize( paraTab )
    ...
    elseif paraTab.AdditionalParameters.luapattern then
        local luaPattern = paraTab.AdditionalParameters.luapattern
        ret = string.match(ret,luaPattern)
        if not ret then
            result = false
            failureMsg = 'match failed'
        end
