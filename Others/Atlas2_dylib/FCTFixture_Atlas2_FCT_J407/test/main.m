...
//    NSDictionary *cal = [Calibration readCalAddress];
//    NSArray *keyDic = [cal allKeys];
//    for (NSString *keyStr in keyDic)
//    {
//        NSLog(@"%@ %@",keyStr,cal[keyStr]);
//    }
    
//    NSString *str = @"[result]: ACK(803.272808425 mV;DONE){u'range': u'5V', u' CH':    u'AI7', u'GND': u'DUT', u'IO': [[41, 1], [45, 1], [126, 0], [125, 0], [30, 1], [124, 0], [123, 0], [122, 0]], u'GAIN': 1}";
//
...
//    NSString *str = @"sensor1";
//
//    NSLog(@"%@",[str substringFromIndex:str.length-1]);
    
    float str =1.0092;
    NSLog(@"[ns==>> %@",[NSNumber numberWithFloat:str]);
    return 0;
}
int main1100()
{
...
//    [cmd setDictionary:[plistParse readAllCMD]];
   ...
   // void * cf = create_fixture_controller(0);
    //NSLog(@"==>> %s",rpc_write_read(cf,"io.get(bit12)",2000,1));
    //const char* rpccmd, int timeout, int site
    //NSLog(@"==>%@",getAndWriteFile(cf, "/mix/addon/dut_firmware/ch1/J407-USBC-2.77.0.2-A1-14-P0-AP-S.bin", "/Users/gdlocal/test/ACE_FW.readback",0,2000));
    //1ae029fb0fd75822f079242b092ab560
    //1ae029fb0fd75822f079242b092ab560
    
//    NSString *bit_name = @"Reset";
//    NSString *bit_state = @"TO_GND_SENSE";
//    if ([cmd objectForKey:kRelayTable])
//    {
//        if ([[cmd objectForKey:kRelayTable] objectForKey:bit_name])
//        {
//            NSArray *valArr = [[cmd objectForKey:kRelayTable] objectForKey:bit_name];
//            for (int i=0; i<[valArr count]; i++)
//            {
//                if ([[valArr[i] uppercaseString] containsString:@"DELAY:"])
//                {
//                    NSArray *arryDelay=[valArr[i] componentsSeparatedByString:@":"];
...
//
   // NSLog(@"===>>>cmd: %@",valStr);
//    void * cf = create_fixture_controller(0);
//    get_relay_table(cf, "reset", "TO_GND_SENSE", 0);
    
    
    //int i=4;
     //NSString * pathLogFile = [[cmd objectForKey:kFIXTUREHWLog] objectForKey:[NSString stringWithFormat:@"UUT%d",i]];
    //NSLog(@"==>>> %@",pathLogFile);
    
   // NSLog(@"==>>> %@",[cmd valueForKey:kHWIO_Critical]);
    
    
//    NSString *bit_name = [NSString stringWithFormat:@"%s","VBUS_SOFTSTART"];
//       NSString *bit_state = [NSString stringWithFormat:@"%s","CONNECT"];
//
//       NSString *valStr = @"";
//       if ([cmd objectForKey:kRelayTable])
//       {
//           if ([[cmd objectForKey:kRelayTable] objectForKey:bit_name])
//           {
//               if ([[[cmd objectForKey:kRelayTable] objectForKey:bit_name] objectForKey:bit_state])
//               {
//                   NSDictionary *valDic = [[[cmd objectForKey:kRelayTable] objectForKey:bit_name] objectForKey:bit_state];
//                   NSArray *valKeyArr = [valDic allKeys];
//                   for (int i=0; i< [valKeyArr count]; i++)
//                   {
//                       valStr = [valDic valueForKey:valKeyArr[i]];
//
//
//                   }
//
...
//           }
//       }
//    NSString *valStr = @"io.set(bit45=1;bit126=0)";
//    
//    NSArray  *ioArr= [valStr componentsSeparatedByString:@"("];
//    NSString *ioStr = nil;
//    if ([ioArr count]>1)
//    {
//        ioStr = [ioArr[1] componentsSeparatedByString:@")"][0];
//    }
//    
//    if (ioStr)
//    {
//        NSMutableString *critical_io = [NSMutableString string];
//        NSMutableString *normal_io = [NSMutableString string];
//        
//        NSArray *bitArr = [ioStr componentsSeparatedByString:@";"];
//        NSString *io_critical = [cmd valueForKey:kHWIO_Critical];
//        
//        for (NSString *ios in bitArr)
//        {
//            NSString *bit = [ios componentsSeparatedByString:@"="][0];
//            if ([io_critical containsString:bit])
//            {
//                [critical_io appendFormat:@"%@;",ios];
//            }
//            else
//            {
//                [normal_io appendFormat:@"%@;",ios];
//            }
//            
//        }
//        
//        if ([critical_io length]>1)
//        {
//            NSString *io_cmd = [NSString stringWithFormat:@"%@(%@)",ioArr[0],[critical_io substringToIndex:[critical_io length]-1]];
//            NSLog(@"===cmd 1>> %@",io_cmd);
//        }
//        
//        if ([normal_io length]>1)
//        {
//            NSString *io_cmd = [NSString stringWithFormat:@"%@(%@)",ioArr[0],[normal_io substringToIndex:[normal_io length]-1]];
//            NSLog(@"===cmd 2>> %@",io_cmd);
//        }
//        
//    }
...
//    NSString *bit_name = @"BATT_SNS";
//    NSString *bit_state = @"GAIN";
//    /*if ([cmd objectForKey:kRelayTable])
//    {
//        if ([[cmd objectForKey:kRelayTable] objectForKey:bit_name])
//        {
...
//                for (int i=0; i< [valKeyArr count]; i++)
//                {
//                    if ([valKeyArr[i] isEqualToString:kIO])
//                    {
//                        //executeRelayAction(controller, [valDic valueForKey:valKeyArr[i]], site);
//                        NSLog(@"==>> %@",[valDic valueForKey:valKeyArr[i]]);
//                    }
//                }
//            }
//
...
//    //writeFixtureLog([NSString stringWithFormat:@"relay_switch2, net_name: %s  state:%s  site: %d,bit_state:%@",net_name,state,site,bit_state]);
//    if ([cmd objectForKey:kMeasureTable])
//    {
//        if ([[cmd objectForKey:kMeasureTable] objectForKey:bit_name])
//        {
//            if ([[[cmd objectForKey:kMeasureTable] objectForKey:bit_name] objectForKey:bit_state])
//            {
//                NSString *valStr = [[[cmd objectForKey:kMeasureTable] objectForKey:bit_name] objectForKey:bit_state];
//                //executeRelayAction(controller, valStr, site);
//                NSLog(@"===>>> %@",valStr);
//            }
//        }
//    }
...
//            if ([[[cmd objectForKey:kMeasureTable] objectForKey:bit_name] objectForKey:bit_state])
//            {
//                NSString *val = [[[cmd objectForKey:kMeasureTable] objectForKey:bit_name] objectForKey:bit_state];
//                NSLog(@"==>> %@",val);
////                NSArray *valKeyArr = [valDic allKeys];
////                for (int i=0; i< [valKeyArr count]; i++)
////                {
...
//    }
   ...
   // NSLog(@">: %@",cmd);
//    int g_SLOTS = [[cmd objectForKey:kFIXTURESLOTS]intValue];
//    NSLog(@"=> %d",g_SLOTS);
//    NSString *ret = @"ACK(12.0 mV;DONE)";
...
//    NSDictionary *d = [cmd objectForKey:@"RelayTable"][@"HALL_CONTROL"][@"SEL_NORTH"];
//    for (int i=0; i<[[d allKeys] count]; i++)
//    {
//        NSLog(@"%@",[d allKeys][i]);
//    }
    /*NSString *bit_name = @"NEGELOAD_CURRENT";
    NSString *bit_state = @"TO_3mA";
    if ([[cmd objectForKey:kRelayTable] objectForKey:bit_name])
    {
        if ([[[cmd objectForKey:kRelayTable] objectForKey:bit_name] objectForKey:bit_state])
        {
            ...
            for (int i=0; i< [valKeyArr count]; i++)
            {
                if ([valKeyArr[i] isEqualToString:kIO] || [valKeyArr[i] isEqualToString:kGPIO])
                {
                    NSLog(@"==cmd :  %@",[valDic valueForKey:valKeyArr[i]]);
                    
                }
                else if([valKeyArr[i] isEqualToString:kDAC2])
                {
                    
                }
                else if([valKeyArr[i] isEqualToString:kR])
                {
                    
                }
                else if([valKeyArr[i] isEqualToString:kDELAY])
                {
                    [NSThread sleepForTimeInterval:[[valDic valueForKey:valKeyArr[i]] doubleValue]];
                }
   ...
   /* NSString *bit_name = @"TST_CLKOUT";
    if ([cmd objectForKey:kMeasureTable])
    {
        if ([[cmd objectForKey:kMeasureTable] objectForKey:bit_name])
        {
            NSDictionary *valDic = [[cmd objectForKey:kMeasureTable] objectForKey:bit_name];
            
            if ([valDic objectForKey:kIO])
            {
                NSLog(@"-> %@",[valDic objectForKey:kIO]);
            }
            if([valDic objectForKey:kGPIO])
            {
                NSLog(@"-> %@",[valDic objectForKey:kGPIO]);
            }
            if([valDic objectForKey:kCH])
            {
                NSLog(@"-> %@",[valDic objectForKey:kCH]);
            }
            if([valDic objectForKey:kGPIO_DISCONNECT])
            {
                 NSLog(@"-> %@",[valDic objectForKey:kGPIO_DISCONNECT]);
            }
   ...
   // NSLog(@"%@",dic[@"RelayTable"][@"DMM_GND"][@"TO_GND"][@"IO"]);
...
//        }
        ...
        NSFileManager *fm = [NSFileManager defaultManager];
        NSError *error = nil;
        NSString *fctFixtureLog=[NSString stringWithFormat:@"/vault/atlas/IAFixture_%@.txt",Time2];
        BOOL isExist = [fm fileExistsAtPath:fctFixtureLog];
        if (!isExist) {
            BOOL ret = [fm createFileAtPath:fctFixtureLog contents:nil attributes:nil];
            if (ret) {
                ...
                [fm createFileAtPath:fctFixtureLog contents:nil attributes:nil];
        ...
        NSFileHandle* fh=[NSFileHandle fileHandleForWritingAtPath:fctFixtureLog];//以只写的方式
        [fh seekToEndOfFile];//将读写指针设置在文件末尾
