...
def test_get_log(rpc_client_wrapper, rpc_server_wrapper):
    '''
    test for get_log and get_and_write_log(log_file)
    These 2 API get log files for current RPC server.
    '''
    # get log folder
    ret, data = rpc_client_wrapper.get_log(timeout_ms=5000)
    assert ret == 'PASS'
    assert data
    ...
    ret = rpc_client_wrapper.get_and_write_log(new_file_path, timeout_ms=3000)
    assert ret == 'PASS'
...
def test_get_linux_boot_log(rpc_client_wrapper):
    import os
    import base64
    ...
    with mock.patch.object(RPCClient, 'get_proxy') as mock_proxy:
        ...
        mock_return_value = rpc_client_wrapper.get_linux_boot_log(mock_file)
