...
def test_customized_threadpool_size(transport, protocol, dispatcher):
    '''
    default pool size is 15; user can specify poolsize.
    '''
    # check default poolsize
    server = RPCServer(transport, protocol, dispatcher)
    assert(server.threadpool._max_workers == 15)

    server = RPCServer(transport, protocol, dispatcher, 20)
    assert(server.threadpool._max_workers == 20)
