...
- (void)testClientThreadSafe
{
    ...
    // verify clients could be created in parallel
    void (^create_rpc_client)(int port) = ^(int port){
        RPCClientWrapper* client = [RPCClientWrapper initWithIP:@"127.0.0.1" withPort:port];
    };
    NSMutableArray* blocks = [NSMutableArray array];

    for (int i = 0; i < num; i++)
    {
        __auto_type block = dispatch_block_create(DISPATCH_BLOCK_NO_QOS_CLASS, ^(void){create_rpc_client(5556 + i);});
