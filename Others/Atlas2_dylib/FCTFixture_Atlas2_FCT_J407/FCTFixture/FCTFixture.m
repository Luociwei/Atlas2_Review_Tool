...
{
    NSString *plist_path = @"/tmp/FCT_Atlas2_calibration.plist";
    @try
    {
        ...
        for (NSString *strKey in [calAddr allKeys])
        {
            ...
            else
            {
                ...
                @try
                {
                    k = [[fixture stringMatch:ret withCount:0] floatValue];
                    if (k<=0.95|| k>=1.05){ k=1;}
                    r = [[fixture stringMatch:ret withCount:1] floatValue];
                }
        ...
        [globalDic writeToFile:plist_path atomically:YES];
    }
    ...
    @catch (NSException *exception)
    {
        ...
        {
            NSDictionary *tmpDic = [[NSDictionary alloc] initWithContentsOfFile:plist_path];
            [globalDic setDictionary:tmpDic];
        }
    }
...
void* create_fixture_controller(int index)
{
    ...
    cmdDic = [NSMutableDictionary dictionary];
    [cmdDic setDictionary:[plistParse readAllCMD]];
    
    globalDic = [NSMutableDictionary dictionary];
...
//    }
    ...
    NSThread *thread3 = [[NSThread alloc] initWithBlock:^{
        [fixture uartShutdown:2];
    }];
    [thread3 start];
    
    NSThread *thread4 = [[NSThread alloc] initWithBlock:^{
        [fixture uartShutdown:3];
    }];
    [thread4 start];
    
    NSTimeInterval starttime = [[NSDate date]timeIntervalSince1970];
    double timeout = 20.0; //s
    while (1)
    {
...
int init(void* controller)
{
    system("pkill -9 virtualport");
    system("pkill -9 usbfs-2.4.20");
    system("rm /tmp/FCT_calibration_Suncode_uut1.txt");
    system("rm /tmp/FCT_calibration_Suncode_uut2.txt");
    system("rm /tmp/FCT_calibration_Suncode_uut3.txt");
    system("rm /tmp/FCT_calibration_Suncode_uut4.txt");
...
float read_voltage(void* controller, const char* net_name,const char* mode,int site)
{
    ...
    if ([[NSString stringWithUTF8String:mode] containsString:@"zero"] && [bit_name isEqualToString:@"PPVBUS_ORION_CONN"])
    {
        bit_name =@"PPVBUS_ORION_CONN_S";
    }
