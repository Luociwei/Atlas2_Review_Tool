...
function func.eloadcurr_cv( param )
    ...
    local cmd= "blade.dac_set(b,"..tostring(vol)..")"
    fixture.rpc_write_read(cmd,timeout,slot_num)

    cmd = "blade.set_cv(84,H)"
    local res = fixture.rpc_write_read(cmd,timeout,slot_num)

    local ret = string.match(res,"ACK%((.+)%,time")
    local value = -999
    if ret then
        value = ret
    end
...
function func.BellaTrix_Reset_LOOP( param )
    ...
    for i=1,5 do
        ...
        local orion_pwr = tonumber(dmm.dmm("PPVBUS_ORION_CONN",param,"mV"))--fixture.read_voltage("PPVBUS_ORION_CONN",slot_num)
...
function func.eloadcurr_step_loop( param )
    ...
    elseif param.AdditionalParameters.param2 ~= nil then
        ...
        for i=start,stop,step do
            ...
            local cmd1 = "eload.set(1,cc,"..tostring(i)..")"
            eload.seteccload({Commands=cmd1,AdditionalParameters={testname=testname,subtestname=subtestname,subsubtestname=subsubtestname,record="NO"}})
            os.execute("sleep 0.02")
            Eload_Current=Eload_Current_Step
            Eload_Current_Step = eload.read_eload_current({Commands="ELOAD_CURRENT_SENSE1",AdditionalParameters={testname=testname,subtestname=subtestname,subsubtestname=subsubtestname,netname="ELOAD_CURRENT_SENSE1",record="NO"}})

            local OC_flag = 0
            local Orion_Voltage =tonumber(dmm.dmm("PPVBUS_ORION_CONN",param,"mV"))--fixture.read_voltage("PPVBUS_ORION_CONN",slot_num)
            ...
            if target then  --Orion OC
                if Eload_Current_Step< target*1000 or Orion_Voltage < 1000 then --171110 IL5 as Alan
                    ...
                    ORION_V=tonumber(dmm.dmm("PPVBUS_ORION_CONN",param,"mV"))*1000
                    OC_flag = OC_flag+1
                end
            else        --Accessory OC
            ...
            if OC_flag>0 then
                if i == start then                 
                    ...
                    local cmd2 = "eload.set(1*cc*0)"
                    eload.seteccload({Commands=cmd2,AdditionalParameters={testname=testname,subtestname=subtestname,subsubtestname=subsubtestname,record="NO"}})
                    value =  i-step
                    result = true
                    break
                end
        ...
        --vt.setVar("ORION_V",ORION_V)
        --vt.setVar("BellaTrix_Status",BellaTrix_Status)
...
function func.highspeed( param )
    ...
    elseif string.find(command,"ABCD") then
        ...
        local default_delimiter = "] :-)"
        dut.setDelimiter("")
        _last_diags_response = _last_diags_response..dut_read(3)
        Log.LogInfo("uart_set.write(ABCD): 0 :".._last_diags_response)
        fixture.rpc_write_read("uart_set.write(ABCD)",5000,slot_num)
        _last_diags_response = _last_diags_response..dut_read(3)

        Log.LogInfo("uart_set.write(ABCD): 1 :".._last_diags_response)
        ...
        if not string.find(_last_diags_response,"Test PASSed") then
            ...
            for i =1,15 do
                ...
                fixture.rpc_write_read("uart_set.write(ABCD)",5000,slot_num)
                _last_diags_response = _last_diags_response..dut_read(2)
        ...
        fixture.rpc_write_read("uart_set.write(EXIT)",5000,slot_num)
        _last_diags_response =_last_diags_response..dut_read(2)

        Log.LogInfo("uart_set.write(ABCD): 2 :".._last_diags_response)
...
function func.sendcmd( param )
    ...
    fixture.rpc_write_read("uart_set.write(\r\n)",5000,slot_num)
    os.execute("sleep 0.5")
    fixture.rpc_write_read("uart_set.write("..command.."\r\n)",5000,slot_num)
...
function func.readvolt( param )
    ...
    local netname = param.AdditionalParameters.netname
    local value = dmm.readvolt(netname,param,nil)
