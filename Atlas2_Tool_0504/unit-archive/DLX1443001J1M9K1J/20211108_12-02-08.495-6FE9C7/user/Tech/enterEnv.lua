...
local ENV_PROMPT_TABLE = { ["diags"] = "] :-) ", ["rtos"] = "SEGPE>", ["rbm"] = " <- Empty ok", ["phleet"] = "CPU0: ( Empty ) ok", ["fsboot"] = "# ", ["iboot"] = "[m]", ["ibootPrompt"] = "] ", ["fixture"] = "_done", [ENV_UNKNOWN] = ""}
local ENV_PROMPT_REGEX_TABLE = { ["diags"] = "%] :%-%)", ["rtos"] = "SEGPE>", ["rbm"] = "%->[%w%s]<%- Empty ok", ["phleet"] = "CPU[%d]+: %( Empty %) ok", ["fsboot"] = "# ", ["iboot"] = "%[m%]", ["ibootPrompt"] = "\n%] $", ["fixture"] = "_done", [ENV_UNKNOWN] = "" }
...
--[[ 
    This function detects environment by checking prompts within timeout.
]]--
...
--[[ 
    This function supports manipulate DUT to enter a specified enviroment, such as EFIDiags, RTOS, IOS, ... etc. It detects current environment DUT in, and use reset commands to reset DUT to iBoot and then send boot command to finish enter env.
    :param env: target environment to enter, supports iboot, diags, rtos, phleet, ios
]]--
