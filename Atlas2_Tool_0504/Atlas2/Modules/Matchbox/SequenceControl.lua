...
local function finishParallelGroup(dag, testName, lastNodeInThreads, forkRoot)
    ...
    local toAdd = {nameMergeGlobals, actionMergeTable, {}, forkRoot.unpack(1)}
    for index, v in ipairs(allGlobals) do
        toAdd[index + 4] = v
    end
    local rMergeGlobals = dag.add(table.unpack(toAdd))
    local globals = rMergeGlobals.unpack(1)
    nameMergeConditions = 'parallel-group-end-merge-conditions-pre-'..testName
    -- use forkRoot's conditions as base conditions
    toAdd = {nameMergeConditions, actionMergeTable, {}, forkRoot.unpack(2)}
    for index, v in ipairs(allConditions) do
        toAdd[index + 4] = v
    end
    local rMergeConditions = dag.add(table.unpack(toAdd))
    local conditions = rMergeConditions.unpack(1)
    ...
    for index, test in ipairs(mainItems) do
        ...
        if forkRoot == nil then
            if thread == nil then
                ...
                local r = dag.add(testName, action, plugins, test, globals, conditions)
                prev = r
            else
...
function SequenceControl._executeTechStep(stepInfo, techFunctionTable, globals, locals, conditions, allowEditingStaticConditions)
    ...
    if not isActEnable then
        ...
        local mapping = {
            [SCOPE_LOCAL]=locals,
            [SCOPE_GLOBAL]=globals,
            [SCOPE_CONDITION]=conditions,
            string=stringConst
        }
        for index, input in ipairs(inputs) do
            ...
            if v == nil then
                error(scope..' '..inputVarName..' not defined.')
            else
        ...
        -- give immutable tables to tech function to ensure tech function doesn't modify
        -- them in code.
        local disableWrite = function (table, name, value)
            msg = 'Not allowed to modify variable table; use tech csv Output column to set values.'
            error(msg, 2)
        end
        ...
        if retryCount > 1 then
            ...
            if subsubtestname == nil then subsubtestname = 'Tech_Line_' .. stepInfo.index end
        ...
        if not isActPass then
            ...
            if subsubtestname == nil then subsubtestname = 'Subtest_' .. stepInfo.index .. ' ' .. stepInfo.TestActions end
...
function SequenceControl._executeTechFA(itemInfo, globals, locals, conditions)
    itemInfo.logID = itemInfo.logID or "tech_fa"
...
function SequenceControl._executeTech(itemInfo, globals, conditions, allowEditingStaticConditions)
    local locals = {}
    itemInfo.logID = itemInfo.logID or "main"
    ...
    local testname = itemInfo.testName
    local tech = itemInfo.testTech
    local testNameSuffix = itemInfo.nameSuffix or ''
    local csvToLoad = ''
    if itemInfo.process == "normal" then
        csvToLoad = '/Tech/' .. itemInfo.testTech .. '.csv'
    ...
    if techFunctionTable == nil then SequenceControl.initTechFunctionTable(itemInfo.testTech) end
    local conditionsModule = require("Matchbox/Condition")
...
function SequenceControl._executeTechAndCatchError(func, itemInfo, ...)
    ...
    if not ret[1] then
        ...
        Log.LogError(msg)
        Record.createBinaryRecord(false, testname, subtestname, subsubtestname, Record.trimFailureMsg(msg))
        error(msg)
    else
