...
function func.fwdl_program( param )
    ...
    local binfile = "J407-USBC-2.116.0.1-A1-19-P0-AP-S.bin"
...
function func.read_gpio(param)
    ...
    local ret =fixture.rpc_write_read(io_connect,timeout,slot_num)
    flow_log.writeFlowLog(ret)
    local adg2188_connect = "adg2188.switch(adg2188_4,4,7,1)"
    ret = fixture.rpc_write_read(adg2188_connect,timeout,slot_num)
    flow_log.writeFlowLog(ret)
    ...
    ret = fixture.rpc_write_read(adg2188_disconnect,timeout,slot_num)
    flow_log.writeFlowLog(ret)
    ...
    Timer.tock(testname.." "..subtestname.." "..subsubtestname)
    flow_log.writeFlowLimitAndResult(param,FDFU)
end
...
function func.powerSet( param )
    ...
    local cmd = "blade.dac_step_set(d,0,3198.276475,444.75)"
    local ret =powersupply.set_power({AdditionalParameters={powertype="USB",start="0",stop="14",step="3"}})
...
function func.otp_write_setback( param )
    ...
    powersupply.set_power({AdditionalParameters={powertype="USB"},Commands="5"})
    relay.relay("VBUS_OUTPUT_CTL","ON")
    os.execute("sleep 0.005")
    relay.relay("VDM_CC1","TO_ACE_CC1")
...
function func.usb3_test_only( param )
    ...
    if limitTab then
        ...
        if tonumber(value) > tonumber(upper) or tonumber(value) < tonumber(lower) then
